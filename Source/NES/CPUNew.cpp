...
const Array<Instruction, 256> CPUNew::s_OpcodeLookup = [] {
	...
	lookup[0x00] = { Op::BRK, InstrType::BRK };
...
void CPUNew::StepInstruction()
{
	switch (m_CurInstr.type)
	{
	case InstrType::BRK: BRK(); break;
...
void CPUNew::BRK()
{
	...
	case 3: PushStack(m_PC >> 8); break;
	case 4: PushStack(m_PC); break;
	case 5: PushStack(m_P | STATUS_BREAK); m_P |= STATUS_INTERRUPT_DISABLE; break;
	case 6: m_ReadBuf[0] = Read(0xFFFE); break;
	case 7: m_PC = (Read(0xFFFF) << 8) | m_ReadBuf[0]; DONE(); break;
	}
}
...
void CPUNew::RTI()
{
	...
	case 2: Read(m_PC); break;
	case 3: m_S++; break;
	case 4: m_P = ReadStack() & ~STATUS_BREAK; m_S++; break;
...
void CPUNew::PLP()
{
	...
	case 2: Read(m_PC); break;
	case 3: m_S++; break;
	case 4: m_P = ReadStack() & ~STATUS_BREAK; DONE(); break;
	}
}
...
void CPUNew::AbsoluteReadModifyWrite()
{
	switch (m_InstrCycle)
	{
	case 2: m_Addr = Read(m_PC++); break;
...
void CPUNew::ZeroPageReadModifyWrite()
{
	switch (m_InstrCycle)
	{
	case 2: m_Addr = Read(m_PC++); break;
...
void CPUNew::ZeroPageIndexedRead()
{
	switch (m_InstrCycle)
	{
	case 2: m_Addr = Read(m_PC++); break;
...
void CPUNew::ZeroPageXReadModifyWrite()
{
	switch (m_InstrCycle)
	{
	case 2: m_Addr = Read(m_PC++); break;
...
template <IndexType IT>
void CPUNew::ZeroPageIndexedWrite()
{
	switch (m_InstrCycle)
	{
	case 2: m_Addr = Read(m_PC++); break;
...
template <IndexType IT>
void CPUNew::AbsoluteIndexedRead()
{
	switch (m_InstrCycle)
	{
	case 2: m_Addr = Read(m_PC++); break;
...
void CPUNew::AbsoluteXReadModifyWrite()
{
	switch (m_InstrCycle)
	{
	case 2: m_Addr = Read(m_PC++); break;
...
template <IndexType IT>
void CPUNew::AbsoluteIndexedWrite()
{
	switch (m_InstrCycle)
	{
	case 2: m_Addr = Read(m_PC++); break;
...
void CPUNew::IndexedIndirectRead()
{
	switch (m_InstrCycle)
	{
	case 2: m_Addr = Read(m_PC++); break;
...
void CPUNew::IndexedIndirectReadModifyWrite()
{
	switch (m_InstrCycle)
	{
	case 2: m_Addr = Read(m_PC++); break;
...
void CPUNew::IndexedIndirectWrite()
{
	switch (m_InstrCycle)
	{
	case 2: m_Addr = Read(m_PC++); break;
...
void CPUNew::IndirectIndexedRead()
{
	switch (m_InstrCycle)
	{
	case 2: m_Addr = Read(m_PC++); break;
...
void CPUNew::IndirectIndexedReadModifyWrite()
{
	switch (m_InstrCycle)
	{
	case 2: m_Addr = Read(m_PC++); break;
...
void CPUNew::IndirectIndexedWrite()
{
	switch (m_InstrCycle)
	{
	case 2: m_Addr = Read(m_PC++); break;
...
u8 CPUNew::ReadStack() const
{
	return Read(m_S + STACK_BEGIN);
}

u8 CPUNew::Read(u16 addr) const
{
	return m_Bus->Read(addr);
}

void CPUNew::Write(u16 addr, u8 val) const
{
	m_Bus->Write(addr, val);
}

void CPUNew::SetStatusBit(u8 mask, bool cond)
{
	m_P = (m_P & ~mask) | (cond ? mask : 0);
}

void CPUNew::SetZN(u8 val)
{
	SetStatusBit(STATUS_ZERO, val == 0);
	SetStatusBit(STATUS_NEGATIVE, val & 0x80);
}

void CPUNew::ExecuteOp()
{
	u8 result;
	u16 result16;
